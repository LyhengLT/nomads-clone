<template>
  <div class="view-container" :class="{ 'lh-filter-active': filterOpen }" v-show="!loading">
    ...
    <!-- Filter Toggle -->
    <div class="lh-filter-toggle" @click="$emit('toggleFilter')">
      {{ filterOpen ? 'Close filter' : 'Filters' }}
    </div>

    <!-- Search -->
    <div class="search-container" id="searchContainer"
         :class="{ 'is-active': searchOpen, 'is-searching': searchQuery.length > 0 }"
         @click.stop="searchOpen = true">
      <div class="search-filter">
        <input type="text" id="searchInput"
               :placeholder="searchOpen ? 'Type to search' : 'Search or filter'"
               v-model="searchQuery"
               @click.stop="searchOpen = true">
        <div class="search-plus">+</div>
      </div>
      <div class="search-dropdown" v-if="searchOpen" @click.stop>
        <SearchFilters />
      </div>
    </div>

    <!-- CGS Wrapper -->
    <div class="cgs-wrapper">
      <div class="compare"
           @click="toggleCompare"
           :class="{ 'compare-active': isCompareActive }">
        {{ compareLabel }}
      </div>
      <div class="grid-view tooltip-set" data-tooltip="Switch between grid, map and chart views">
        <select class="grid-view-1">
          <option v-for="label in controls.gridView" :key="label"
                  :selected="label === controls.gridViewDefault">{{ label }}</option>
        </select>
      </div>
      <div class="cgs-controls tooltip-set" data-tooltip="Sort cities by different scores">
        <select class="nomads-score">
          <optgroup v-for="group in controls.nomadScore" :key="group.group" :label="group.group">
            <option v-for="opt in group.options" :key="opt"
                    :selected="opt === controls.nomadScoreDefault">{{ opt }}</option>
          </optgroup>
        </select>
...
import LogoDropdown from './LogoDropdown.vue'
...
const compareLabel = ref('Compare')
const isCompareActive = ref(false)

function toggleCompare() {
  isCompareActive.value = !isCompareActive.value
  compareLabel.value = isCompareActive.value ? 'Exit Compare' : 'Compare'
}
